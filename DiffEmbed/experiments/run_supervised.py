...
class DefaultCollator:
    ...
    def __init__(self, model: LLM2Vec, use_repllama_loss: bool = False) -> None:
        """
        Constructor for DefaultCollator.

        Args:
            model (LLM2Vec): The model instance.
            use_repllama_loss (bool): Whether to use the RepllamaLoss approach for collating data.
        """
        self.model = model
        self.use_repllama_loss = use_repllama_loss

    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        """
        Collates data for the batch.

        Args:
            features (List[Dict[str, Any]]): A list of feature dictionaries, where each dictionary
                                             contains the texts and labels for a sample.
...
class RepllamaLLM2VecSupervisedTrainer(LLM2VecSupervisedTrainer):
   def compute_loss(
        self,
        model: nn.Module,
        inputs: Dict[str, Union[torch.Tensor, Any]],
        ...
        for i in range(2,len(features)):
            d_reps_neg = model(features[i])
            d_reps_negs.append(d_reps_neg)

        custom_loss = self.loss_function(q_reps, d_reps, d_reps_negs)

        if return_outputs:
            output = torch.cat(
                [model(row)["sentence_embedding"][:, None] for row in features], dim=1
            )
            return custom_loss, output
...
def main():
    ...
    if use_repllama_loss:
        train_loss = load_loss(custom_args.loss_class, scale_by_dim=custom_args.scale_by_dim)
    else:
        train_loss = load_loss(custom_args.loss_class, scale=custom_args.loss_scale)

    data_collator = DefaultCollator(model, use_repllama_loss)
    
    trainer_cls = RepllamaLLM2VecSupervisedTrainer if use_repllama_loss else LLM2VecSupervisedTrainer
    trainer = trainer_cls(
        model=model,
        args=training_args,
